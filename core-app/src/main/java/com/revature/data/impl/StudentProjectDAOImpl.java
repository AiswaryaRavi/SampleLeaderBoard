...
public class StudentProjectDAOImpl implements StudentProjectDAO {
	...
	@Override
	public List<StudentProject> getStudentProjectPercentageCalculation(Integer studentId) throws DataServiceException {
		List<StudentProject> studentProjects = null;
		try {
			StringBuilder sb = new StringBuilder("SELECT NAME,ROUND(PERCENTAGE)AS PERCENTAGE FROM `vw_project_activity_percentage_calculation` WHERE STUDENT_ID=" + studentId);
			studentProjects = dataRetriver.retrieveBySQLJSON(sb.toString());
			logger.info("Student Projects percentage data retrieval success.."+studentProjects);
		} catch (DataAccessException e) {
			logger.error(e.getMessage(), e);
			throw new DataServiceException(DataUtils.getPropertyMessage("data_retrieval_fail"), e);
		}
